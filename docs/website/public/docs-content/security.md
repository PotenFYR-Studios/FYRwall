...
none, nosniff, no-referrer). Remote agents use mTLS client certificates plus
per-agent bearer credentials. Certificates rotate automatically and are
individually revocable; server stores credential hashes only.
